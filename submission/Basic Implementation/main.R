...
#' @description Help to calculate the OLS estimators generate by using Boston Housing dataset.
#' @param data A data frame containing variables (e.g.Boston Housing dataset.)
#' @param respond A string contains selecting variable name.
#' @param predictor A string contains selecting variable name.
#' @return A list containing: beta0, beta1, ols_conf_int (95% confidence interval).
...
#' respond <- "medv"
#' predictor <- "lstat"
#' test_index <- 1:nrow(data)
#' boots <- boot_fn(data, index = test_index, predictor, respond)
#' head(boots)
#' @export
...
#' test_index <- 1:nrow(data)
#' boots <- boot_fn(data, index = test_index, predictor, respond)
#' boot_result <- bootstrap_slr_summary(data, predictor, respond, R = 1000,seed = NULL, boot_fn = boot_fn)
#' print(boot_result)
#' @export
bootstrap_slr_summary <- function(data, predictor, respond, R = 1000,seed = NULL,
                                  boot_fn){  
  if (!is.null(seed)) {
    set.seed(seed)
  }else{
  ...
  boot_model<- boot(data = data,
                    statistic = boot_fn,
                    R = R, predictor = predictor, respond = respond)
  boot_slr <- list(
    ...
    boot_b0_bias = mean(boot_model$t[, 1]) - boot_model$t0[1],
    boot_b1_bias = mean(boot_model$t[, 2]) - boot_model$t0[2],
...
#' predictor <-"lstat"
#' ols_slr <- ols_estimators(data, predictor, respond)
#' boot_slr <- bootstrap_slr_summary(data, predictor, respond, R = 1000,seed = NULL, boot_fn = boot_fn)
#' table <- bootstrap_slr(boot_slr, ols_slr)
#' print(table)
#' @export
...
#' @param col_ols Color of OLS reference line. Default "red"
...
#' predictor <-"lstat"
#' ols_slr <- ols_estimators(data, predictor, respond)
#' boot_slr <- bootstrap_slr_summary(data, R = 1000, seed = NULL, predictor, respond, boot_fn = boot_fn)
